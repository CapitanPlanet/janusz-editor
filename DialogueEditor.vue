...
import { readTextFile, writeTextFile, copyFile, BaseDirectory, exists, mkdir } from '@tauri-apps/plugin-fs'
...
const resolveImagePath = async (path: string): Promise<string> => {
  ...
  // Ścieżki z backgrounds/ w AppData
  ...
  console.log('[IMG] Próbuję public/:', path)
  return path
}
...
const ensureDir = async (path: string) => {
  try {
    ...
    if (!dirExists) {
      ...
      console.log('[FS] Stworzono katalog:', path)
    }
  } catch (e) {
    console.error('[FS] Błąd tworzenia katalogu:', e)
    throw e
  }
}
...
const pickBackground = async () => {
  ...
  const fileName = selected.split('\\').pop()?.split('/').pop() || `bg_${Date.now()}.jpg`
  const relativePath = `backgrounds/${fileName}`

  try {
    await ensureDir('backgrounds')
    console.log('[PICK] Kopiuję do:', relativePath)
    await copyFile(selected, relativePath, { toPathBaseDir: BaseDirectory.AppData })
    scenes.value[selectedSceneIndex.value].Background = relativePath
    delete imageCache.value[relativePath]
    console.log('[PICK] SUKCES! Zapisano ścieżkę:', relativePath)
    alert(`Skopiowano: ${fileName}`)
  } catch (e) {
    console.error('[PICK] BŁĄD KOPIOWANIA:', e)
    scenes.value[selectedSceneIndex.value].Background = selected
    alert(`Błąd kopiowania! Używam ścieżki absolutnej.\nSprawdź uprawnienia w tauri.conf.json\n\nBłąd: ${e}`)
  }
}
...
const pickReactionImage = async (choiceIndex: number) => {
  ...
  const fileName = selected.split('\\').pop()?.split('/').pop() || `reaction_${Date.now()}.jpg`
  const relativePath = `backgrounds/${fileName}`

  try {
    await ensureDir('backgrounds')
    await copyFile(selected, relativePath, { toPathBaseDir: BaseDirectory.AppData })
    scenes.value[selectedSceneIndex.value].Choices[choiceIndex].ReactionImage = relativePath
    delete imageCache.value[relativePath]
  } catch (e) {
    console.error('[PICK] Błąd kopiowania:', e)
    scenes.value[selectedSceneIndex.value].Choices[choiceIndex].ReactionImage = selected
    alert(`Błąd kopiowania! Używam ścieżki absolutnej.\nBłąd: ${e}`)
  }
}
</script>
